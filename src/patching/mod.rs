...
#[repr(transparent)]
pub struct ModuleRef(HINSTANCE);

impl ModuleRef {
    pub fn new(handle: HINSTANCE) -> Self {
        ModuleRef(handle)
    }
...
impl Module {
    ...
    /// # Safety
    /// The caller must ensure the `ptr` is within the bounds of the module & parent process.
    pub unsafe fn write_absolute_buffer<T>(
        &mut self,
        ptr: *mut u8,
        buffer: &mut [u8],
    ) -> Result<()> {
...
impl TryFrom<Module> for LocalModule {
    ...
    fn try_from(module: Module) -> std::result::Result<Self, Self::Error> {
        let current_process = GameProcess::current_process();

        if module.parent.pid == current_process.pid {
            Ok(Self(module))
        } else {
